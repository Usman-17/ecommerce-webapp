...
const AreaFilter = ({
  ...
  if (isLoading) return <AreaFilterSkeleton />;
  ...
  const displayItems =
    itemLimit > 0 && !showAll
      ? filteredAreas.slice(0, itemLimit)
      : filteredAreas;
  const hasMore = itemLimit > 0 && filteredAreas.length > itemLimit;
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3 pt-2 pr-2">
        {filteredAreas.length === 0 ? (
          ...
          displayItems.map((area) => {
            ...
            const productCount = getAreaProductCount
              ? getAreaProductCount(area.name)
              : 0;

            return (
              <div
                key={area._id || area.key}
      ...
      {hasMore && (
        ...
        >
          {showAll
            ? "View Less"
            : `View More (${filteredAreas.length - initialItemCount})`}
        </button>
      )}
    </div>
  );
