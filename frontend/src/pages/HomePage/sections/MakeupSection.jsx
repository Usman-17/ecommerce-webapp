...
const MakeupSection = () => {
  ...
  const makeupProducts = products.filter(
    (p) =>
      p.productAreaName?.toLowerCase() === "makeup & beauty" ||
      p.productAreaName?.toLowerCase().includes("makeup"),
  );

  useEffect(() => {
    if (scrollRef.current && makeupProducts.length > 0) {
  ...
  return (
    <section className="pt-8 pb-16 sm:pt-12 sm:pb-6 lg:px-32">
      <div className="max-w-480 mx-auto">
        <SectionHeader
          label="Makeup & Beauty"
          title="Glow Up Your Look"
          description="Discover premium makeup and beauty products to enhance your natural beauty."
          viewAllLink="/shop?area=Makeup%20%26%20Beauty"
        />
