...
const formatDeal = (deal) => ({
  ...
  productIds: deal.products?.map((p) => p._id?.toString?.() || p.toString?.()) || [],
...
export const updateDeal = async (req, res) => {
  try {
    const { id } = req.params;
    const { title, description, dealPrice, originalPrice, products, isActive, removedImages } =
      req.body;
    ...
    if (originalPrice !== undefined) deal.originalPrice = originalPrice ? Number(originalPrice) : undefined;
    ...
    if (removedImages) {
      ...
      if (removeIds.length > 0) {
        ...
        deal.images = deal.images.filter((img) => !removeIds.includes(img.public_id));
      }
    }
