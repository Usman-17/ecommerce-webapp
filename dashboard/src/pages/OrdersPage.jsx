...
const OrdersPage = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        const customerName =
          `${record.address?.firstName || ""} ${record.address?.lastName || ""}`.trim() ||
          record.customerName ||
          "Customer";
        const phone = record.address?.phone || record.customerPhone || "";
        const trackingNo = record.trackingNo || "";
        const totalAmount = Math.floor(
          record.amount + (record.shippingCharge || 0),
        ).toLocaleString();
        const msg = `Hi ${customerName},%0AThank you for shopping with Jemzy.%0AYour order has been successfully received and is now being processed.%0A%0AOrder No: ${trackingNo}%0ATotal Amount: Rs. ${totalAmount}%0A%0AWe will notify you once your order is confirmed and ready for dispatch.%0A%0AIf you have any questions, feel free to contact our support team.%0A%0AThank you for choosing Jemzy.`;
        ...
        return (
          <div className="flex items-center justify-center gap-2">
            <a
              ...
              title="Send WhatsApp"
              className="p-2 rounded-full border border-green-300 bg-white text-green-600 hover:bg-green-50 transition-colors"
            >
